...
local MhdxrHubConfig = Window.ConfigManager:CreateConfig("Mhdxrhub")
...
local function Reg(id, element)
    MhdxrHubConfig:Register(id, element)
...
local BaseFolder = "WindUI/" .. (Window.Folder or "MhdxrHub") .. "/config/"
...
do
    ...
    _G.MhdxrHub_AutoAcceptTradeEnabled = false 
    ...
    if PromptController and PromptController.FirePrompt and Promise then
        ...
        PromptController.FirePrompt = function(self, promptText, ...)
            ...
            if _G.MhdxrHub_AutoAcceptTradeEnabled and type(promptText) == "string" and promptText:find("Accept") and promptText:find("from:") then
        ...
        warn("[MhdxrHub] Gagal memuat PromptController/Promise untuk Auto Accept Trade.")
    end
end
...
do
    ...
    home:Section({
        Title = "Join Discord Server MhdxrHub",
        TextSize = 18,
    })

    home:Paragraph({
        Title = "MhdxrHub Community",
        ...
        Buttons = {
            {
                ...
                Icon = "link",
                Callback = function()
                    setclipboard("https://dsc.gg/Mhdxrhub")
                    WindUI:Notify({
                        Title = "Link Disalin!",
                        Content = "Link Discord MhdxrHub berhasil disalin.",
                        Duration = 3,
                        Icon = "copy",
                    })
                end,
        ...
        if data then
            ...
            if targetPlayer.Character then
                for _, v in ipairs(targetPlayer.Character:GetChildren()) do
                    if v.Name == "MhdxrHubESP" and v:IsA("BillboardGui") then pcall(function() v:Destroy() end) end
                end
            end
        end
    end
...
do
    ...
    _G.MhdxrHub_BlatantActive = false
    ...
    task.spawn(function()
        ...
        if S1 and FishingController then
            ...
            FishingController.RequestChargeFishingRod = function(...)
                if _G.MhdxrHub_BlatantActive then return end 
                return Old_Charge(...)
            end
            FishingController.SendFishingRequestToServer = function(...)
                if _G.MhdxrHub_BlatantActive then return false, "Blocked by MhdxrHub" end
                return Old_Cast(...)
            end
        end
    end)
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if _G.MhdxrHub_BlatantActive and not checkcaller() then
    ...
    local function SuppressGameVisuals(active)
        ...
        if active then
            task.spawn(function()
                ...
                while _G.MhdxrHub_BlatantActive do
    ...
    local togblat = Reg("blatantt",blatant:Toggle({
        ...
        Callback = function(state)
            ...
            _G.MhdxrHub_BlatantActive = state
...
local function GetItemsToFavorite()
    ...
    local accept = trade:Toggle({
        ...
        Value = false,
        Callback = function(state)
            _G.MhdxrHub_AutoAcceptTradeEnabled = state
...
do
    ...
    local function CreateKaitunUI()
        local old = game.CoreGui:FindFirstChild("MhdxrHubKaitunStats")
        if old then old:Destroy() end
        local sg = Instance.new("ScreenGui")
        sg.Name = "MhdxrHubKaitunStats"
        ...
        txt("KAITUN MhdxrHUB (PREMIUM)", 0.2, Color3.fromRGB(255,0,255), 35)
        ...
        Callback = function(state)
            ...
            if state then
                ...
                if not _G.BlackScreenGUI then
                    ...
                    _G.BlackScreenGUI.Name = "MhdxrHub_BlackBackground"
        ...
        Callback = function(state)
            ...
            if state then
                ...
                for _, gui in ipairs(PlayerGui:GetChildren()) do
                    if gui:IsA("ScreenGui") and gui.Name ~= "WindUI" and gui.Name ~= "CustomFloatingIcon_MhdxrHub" then
...
do
    ...
    local WEBHOOK_URL = ""
    local WEBHOOK_USERNAME = "MhdxrHub Notify" 
    ...
    local GLOBAL_WEBHOOK_USERNAME = "MhdxrHub | Community"
    ...
    local function onFishObtained(itemId, metadata, fullData)
        local success, results = pcall(function()
            ...
            if isWebhookEnabled and WEBHOOK_URL ~= "" and isUserFilterMatch then
                local title_private = string.format("<:TEXTURENOBG:1438662703722790992> MhdxrHub | Webhook\n\n<a:ChipiChapa:1438661193857503304> New Fish Caught! (%s)", fishName)
                ...
                local embed = {
                    ...
                    footer = {
                        text = string.format("MhdxrHub Webhook • Total Caught: %s • %s", caughtDisplay, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
            ...
            if isGlobalTarget and GLOBAL_WEBHOOK_URL ~= "" then 
                ...
                local title_global = string.format("<:TEXTURENOBG:1438662703722790992> MhdxrHub | Global Tracker\n\n<a:globe:1438758633151266818> GLOBAL CATCH! %s", fishName)
                ...
                local globalEmbed = {
                    ...
                    thumbnail = { url = imageUrl },
                    footer = {
                        text = string.format("MhdxrHub Community| Player: %s | %s", censoredPlayerName, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
        ...
        if not success then
            warn("[MhdxrHub Webhook] Error processing fish data:", results)
        end
    end
...
do
    ...
    local SelectedConfigName = "Mhdxrhub" -- Default
    local BaseFolder = "WindUI/" .. (Window.Folder or "MhdxrHub") .. "/config/"
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            ...
            -- 1. Save ke config utama dulu ("Mhdxrhub.json")
            MhdxrHubConfig:Save()
            task.wait(0.1)

            -- 2. Jika nama beda, salin isi "Mhdxrhub.json" ke "NamaBaru.json"
            if SelectedConfigName ~= "Mhdxrhub" then
                local success, err = pcall(function()
                    local mainContent = readfile(BaseFolder .. "Mhdxrhub.json")
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            if SelectedConfigName == "" or SelectedConfigName == "Mhdxrhub" then 
            ...
            if isfile(path) then
                ...
                RefreshConfigList(ConfigDropdown)
                ConfigNameInput:Set("Mhdxrhub")
                SelectedConfigName = "Mhdxrhub"
            else
    ...
    --ConfigSection:Paragraph({
       ...
       -- Desc = "Script otomatis menyimpan ke 'Mhdxrhub.json' setiap 30 detik.",
      --  Icon = "info"
 --   })
end
...
WindUI:Notify({ Title = "MhdxrHub Was Loaded", Content = "Press [F] to open/close the menu", Duration = 5, Icon = "info" })
-- [[ AUTO LOAD & SAVE LOOP ]]
task.spawn(function()
    task.wait(2) -- Tunggu UI load sempurna
    
    -- Ganti Load biasa dengan Smart Load
    -- Default load "Mhdxrhub", atau nama config terakhir user kalau kamu simpan
    SmartLoadConfig("Mhdxrhub") 
    
    -- Auto Save Loop (Setiap 30 detik) -- Save tetap pakai cara biasa gapapa
    while true do
         task.wait(10)
         pcall(function() MhdxrHubConfig:Save() end)
    end
end)
InitializeIcon()